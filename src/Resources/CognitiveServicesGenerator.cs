...
namespace Provision
{
    internal class CognitiveServicesGenerator : IResourceGenerator
    {
        ...
        public string GenerateEnvScript() => $@"{cognitiveServices.CognitiveServicesEndpointVariable}=`az cognitiveservices account show -g ${cognitiveServices.ResourceGroup.ResourceGroupNameVariable} -n ${cognitiveServices.CognitiveServicesAccountNameVariable} --query ""endpoint"" -o tsv`
